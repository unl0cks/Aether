...
/// AQW references several legacy macOS/pixel device-font family names that
/// are normally absent on Windows. Returning a deterministic metrically close
/// family is better than whichever previously loaded device font happens to
/// win Ruffle's generic fallback search.
fn aqw_device_font_alias(name: &str) -> Option<(&'static str, fontdb::Weight)> {
    if name.eq_ignore_ascii_case("Helvetica Neue Bold") {
        Some(("Arial Nova", fontdb::Weight::BOLD))
    } else if name.eq_ignore_ascii_case("Helvetica Light") {
        Some(("Arial Nova", fontdb::Weight::LIGHT))
    } else if name.eq_ignore_ascii_case("Mini 7") {
        Some(("Tahoma", fontdb::Weight::NORMAL))
    } else if name.eq_ignore_ascii_case("Mini 7 Tight") {
...
#[cfg(test)]
mod aqw_device_font_alias_tests {
    use super::*;

    #[test]
    fn aqw_missing_device_fonts_use_deterministic_windows_compatible_aliases() {
        assert_eq!(
            aqw_device_font_alias("Helvetica Neue Bold"),
            Some(("Arial Nova", fontdb::Weight::BOLD))
        );
        assert_eq!(
            aqw_device_font_alias("Helvetica Light"),
            Some(("Arial Nova", fontdb::Weight::LIGHT))
        );
        assert_eq!(
            aqw_device_font_alias("Mini 7"),
            Some(("Tahoma", fontdb::Weight::NORMAL))
        );
        assert_eq!(
            aqw_device_font_alias("Mini 7 Tight"),
            Some(("Arial Narrow", fontdb::Weight::NORMAL))
        );
...
impl UiBackend for DesktopUiBackend {
    ...
    fn load_device_font(&self, query: &FontQuery, register: &mut dyn FnMut(FontDefinition)) {
        ...
        let id = self.font_database.query(&exact_query).or_else(|| {
            ...
            let id = self.font_database.query(&alias_query);
            if id.is_some() {
                selected_alias = Some(family_name);
            }
            id
        });
        ...
        {
            let alias_suffix = selected_alias
                .map(|family| format!(" via {family}"))
                .unwrap_or_default();
            tracing::info!(
                "Loading device font \"{}\" for \"{name}\"{alias_suffix} (italic: {is_italic}, bold: {is_bold})",
