...
impl MainWindow {
    pub fn window_event(&mut self, event_loop: &ActiveEventLoop, event: WindowEvent) {
        if matches!(&event, WindowEvent::RedrawRequested) {
            ...
            if !self.minimized {
                ...
                if let Some(ref mut player) = player {
                    ...
                    #[cfg(feature = "metrics")]
                    {
                        player_render_time = started.elapsed();
                ...
                if let Some(mut player) = self.player.get() {
                    player.record_host_render_time(host_render_time);
                }
                ...
                #[cfg(feature = "metrics")]
                {
                    ...
                    self.metrics.record_render(
                        ...
                        gui_present_time,
                        player_rendered,
                        gui_render_outcome == GuiRenderOutcome::Presented,
                        counters,
                        wgpu,
                    );
                }
        ...
        #[cfg(feature = "metrics")]
        if is_cursor_moved {
            self.metrics.record_host_mouse_received();
        }
        ...
        match event {
            ...
            WindowEvent::CursorMoved { position, .. } => {
                ...
                if position == self.mouse_pos {
                    #[cfg(feature = "metrics")]
                    self.metrics.record_host_mouse_duplicate();
                    return;
                }

                self.mouse_pos = position;
                let decision = self.mouse_motion.push(position, Instant::now());
                #[cfg(feature = "metrics")]
                if decision.coalesced {
                    self.metrics.record_host_mouse_coalesced();
                }
                if let Some(position) = decision.dispatch {
                    self.dispatch_mouse_move(position, true);
                }
            }
            ...
            WindowEvent::MouseInput { button, state, .. } => {
                ...
                #[cfg(feature = "metrics")]
                ruffle_core::aether_diagnostics::record_host_mouse(
                    match state {
    ...
    fn dispatch_mouse_move(&mut self, position: PhysicalPosition<f64>, request_redraw: bool) {
        ...
        #[cfg(feature = "metrics")]
        self.metrics.record_host_mouse_dispatched();
        if request_redraw {
            self.check_redraw();
        }
    }
    ...
    fn flush_pending_mouse_move(&mut self, now: Instant, request_redraw: bool) {
        if let Some(position) = self.mouse_motion.flush(now) {
            self.dispatch_mouse_move(position, request_redraw);
        }
    }

    fn flush_due_mouse_move(&mut self, now: Instant) {
        if let Some(position) = self.mouse_motion.take_due(now) {
            self.dispatch_mouse_move(position, true);
        }
    }
...
impl ApplicationHandler<RuffleEvent> for App {
    ...
    fn user_event(&mut self, event_loop: &ActiveEventLoop, event: RuffleEvent) {
        ...
        match (&mut self.main_window, event) {
            ...
            (Some(main_window), RuffleEvent::CloseFile) => {
                main_window.gui.window().set_title("Aether"); // Reset title since file has been closed.
    ...
    fn about_to_wait(&mut self, event_loop: &ActiveEventLoop) {
        ...
        if let Some(main_window) = &mut self.main_window {
            ...
            if let Some(next_wake_time) = main_window.next_wake_time() {
                event_loop.set_control_flow(ControlFlow::WaitUntil(next_wake_time));
            }
        }
    }
