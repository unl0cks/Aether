...
/// The GUI may place the movie texture at fractional physical coordinates when
/// the window, menu height, or DPI scale is non-integral. Linear sampling keeps
/// text and vector edges stable in those windowed configurations.
fn movie_blit_filter() -> wgpu::FilterMode {
...
impl MovieViewRenderer {
    pub fn new(
        ...
        let blit_filter = movie_blit_filter();
        let sampler = device.create_sampler(&wgpu::SamplerDescriptor {
            mag_filter: blit_filter,
            min_filter: blit_filter,
            ..Default::default()
        });
...
mod tests {
    ...
    #[test]
    fn final_movie_blit_filters_fractional_window_composition() {
        assert_eq!(movie_blit_filter(), wgpu::FilterMode::Linear);
    }
}
