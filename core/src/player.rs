...
pub struct Player {
    ...
    /// Capacity hints learned from previous frames. These are enabled only in Aether builds and
    /// reduce repeated growth allocations without changing command ordering or Flash behavior.
    #[cfg(feature = "aether_render_hints")]
    render_command_capacity_hint: usize,
    #[cfg(feature = "aether_render_hints")]
    ...
    recent_host_render_timings: VecDeque<f64>,
...
impl Player {
    ...
    ///
    /// Frontends may also report their complete render/present duration. That cost must be
    /// reserved before scheduling authored catch-up frames: running five invisible frames after
    /// an already-over-budget render creates an overload feedback loop and gives wall-clock-driven
    /// ActionScript callbacks several identical timestamps.
    fn max_frames_per_tick(&self) -> u32 {
        let average_run_frame_time = average_timing(&self.recent_run_frame_timings);
        let average_host_render_time = average_timing(&self.recent_host_render_timings);
        calculate_max_frames_per_tick(
            self.frame_time(1000.0),
            average_run_frame_time,
            average_host_render_time,
        )
    }
    ...
    /// Report the frontend's complete player-render and presentation time.
    ///
    /// Generic embedders that do not call this retain Ruffle's script-only catch-up heuristic.
    pub fn record_host_render_time(&mut self, elapsed: Duration) {
        self.recent_host_render_timings
            .push_back(elapsed.as_secs_f64() * 1_000.0);
        if self.recent_host_render_timings.len() >= 10 {
            self.recent_host_render_timings.pop_front();
        }
    }
    ...
    pub fn tick(&mut self, dt: FloatDuration) {
        ...
        while frame < max_frames_per_tick && self.frame_accumulator >= frame_duration {
            ...
            crate::aether_metrics::authored_frame_executed();
        ...
        crate::aether_metrics::record_tick_phase(
            crate::aether_metrics::TickPhase::AudioSkew,
        ...
        #[cfg(feature = "aether_metrics")]
        crate::aether_metrics::record_tick_phase(
            crate::aether_metrics::TickPhase::Sockets,
            phase_started.elapsed(),
        );
        ...
        #[cfg(feature = "aether_metrics")]
        crate::aether_metrics::record_tick_phase(
            crate::aether_metrics::TickPhase::NetConnections,
            phase_started.elapsed(),
        );
        ...
        #[cfg(feature = "aether_metrics")]
        crate::aether_metrics::record_tick_phase(
            crate::aether_metrics::TickPhase::Timers,
            phase_started.elapsed(),
        );
        ...
        #[cfg(feature = "aether_metrics")]
        crate::aether_metrics::record_tick_phase(
            crate::aether_metrics::TickPhase::Streams,
            phase_started.elapsed(),
        );
        ...
        #[cfg(feature = "aether_metrics")]
        crate::aether_metrics::record_tick_phase(
            crate::aether_metrics::TickPhase::AudioTick,
            phase_started.elapsed(),
        );
    }
    ...
    pub fn handle_event(&mut self, event: PlayerEvent) -> bool {
        #[cfg(feature = "aether_metrics")]
        let mouse_move_started = matches!(&event, PlayerEvent::MouseMove { .. }).then(Instant::now);
        ...
        #[cfg(feature = "aether_metrics")]
        if let Some(started) = mouse_move_started {
            crate::aether_metrics::record_mouse_move(started.elapsed());
        }
    ...
    fn update_mouse_state(
        ...
        let needs_render = self.mutate_with_update_context(|context| {
            ...
            #[cfg(feature = "aether_diagnostics")]
            let trace_pressed_before_button_update = context.mouse_data.pressed(MouseButton::Left);
            ...
            let needs_render = if events.is_empty() {
                ...
                for (object, event) in events {
                    ...
                    #[cfg(feature = "aether_diagnostics")]
                    let trace_data = if crate::aether_diagnostics::input_trace_enabled() {
                        let hovered_object = context
                            .mouse_data
                            .hovered
                            .map(|hovered| hovered.as_displayobject());
                        let pressed_object = trace_pressed_before_button_update
                            .map(|pressed| pressed.as_displayobject());
                        let target_route =
                            crate::aether_diagnostics::mouse_route_chain(display_object, context);
                        let hovered_route = hovered_object
                            .map(|object| {
                                crate::aether_diagnostics::mouse_route_chain(object, context)
                            })
                            .unwrap_or_default();
                        let pressed_route = pressed_object
                            .map(|object| {
                                crate::aether_diagnostics::mouse_route_chain(object, context)
                            })
                            .unwrap_or_default();
                        Some((
                            crate::aether_diagnostics::clip_event_name(event),
                            crate::aether_diagnostics::DisplayObjectDescriptor::from_display_object(
                                display_object,
                            ),
                            hovered_object.map(
                                crate::aether_diagnostics::DisplayObjectDescriptor::from_display_object,
                            ...
                            hovered_route,
                            pressed_route,
                            context.mouse_position.x.to_pixels(),
                            context.mouse_position.y.to_pixels(),
                        ))
                    } else {
                        None
                    };

                    let (clip_handler_handled, avm2_dispatch_handled) =
                        if !display_object.avm1_removed() {
                            let clip_handler_handled = object.handle_clip_event(context, event)
                                == ClipEventResult::Handled;
                            if clip_handler_handled {
                                *player_event_handled = true;
                            }
                            if matches!(event, ClipEvent::Press { .. }) {
                                Self::update_focus_on_mouse_press(context, display_object);
                            }
                            let avm2_dispatch_handled = object.event_dispatch_to_avm2(context, event)
                                == ClipEventResult::Handled;
                            if avm2_dispatch_handled {
                                *player_event_handled = true;
                            }
                            (clip_handler_handled, avm2_dispatch_handled)
                        } else {
                            (false, false)
                        };

                    #[cfg(not(feature = "aether_diagnostics"))]
                    let _ = (clip_handler_handled, avm2_dispatch_handled);
                    ...
                    {
                        crate::aether_diagnostics::record_core_clip_event(
                            event_name,
                            stage_x,
                            stage_y,
                            target,
    ...
    pub fn run_frame(&mut self) {
        ...
        #[cfg(feature = "aether_metrics")]
        crate::aether_metrics::record_tick_phase(
            crate::aether_metrics::TickPhase::Preload,
            phase_started.elapsed(),
        );
        ...
        self.update(|context| {
            ...
            #[cfg(feature = "aether_metrics")]
            crate::aether_metrics::record_tick_phase(
                crate::aether_metrics::TickPhase::Avm1Frame,
                phase_started.elapsed(),
            );
            ...
            #[cfg(feature = "aether_metrics")]
            crate::aether_metrics::record_tick_phase(
                crate::aether_metrics::TickPhase::SoundUpdate,
                phase_started.elapsed(),
            );
            ...
            #[cfg(feature = "aether_metrics")]
            crate::aether_metrics::record_tick_phase(
                crate::aether_metrics::TickPhase::LocalConnections,
                phase_started.elapsed(),
            );
            ...
            #[cfg(feature = "aether_metrics")]
            crate::aether_metrics::record_tick_phase(
                crate::aether_metrics::TickPhase::PostFrameCallbacks,
                phase_started.elapsed(),
            );
        });
    ...
    pub fn render(&mut self) {
        ...
        let (cache_draws, commands) = self.enter_arena_mut(|gc_context, gc_root, this| {
            ...
            #[cfg(feature = "aether_render_hints")]
            let mut cache_draws = Vec::with_capacity(this.cache_draw_capacity_hint);
            #[cfg(not(feature = "aether_render_hints"))]
            let mut cache_draws = vec![];

            #[cfg(feature = "aether_render_hints")]
            let commands = CommandList::with_capacity(this.render_command_capacity_hint);
            #[cfg(not(feature = "aether_render_hints"))]
            ...
            #[cfg(feature = "aether_render_hints")]
            {
                // Keep enough room for the largest scene seen so far, with a modest 12.5% margin.
                // Cap the hints so a pathological SWF cannot permanently reserve absurd memory.
                const MAX_COMMAND_HINT: usize = 1_000_000;
                const MAX_CACHE_DRAW_HINT: usize = 65_536;

                let command_count = commands.commands.len();
                let command_hint = command_count.saturating_add(command_count / 8);
                this.render_command_capacity_hint = this
                    .render_command_capacity_hint
                    .max(command_hint)
                    .min(MAX_COMMAND_HINT);

                let cache_draw_count = cache_draws.len();
                let cache_draw_hint = cache_draw_count.saturating_add(cache_draw_count / 8);
                this.cache_draw_capacity_hint = this
...
fn average_timing(samples: &VecDeque<f64>) -> Option<f64> {
    (!samples.is_empty()).then(|| samples.iter().sum::<f64>() / samples.len() as f64)
}

fn calculate_max_frames_per_tick(
    frame_time_ms: f64,
    average_run_frame_ms: Option<f64>,
    average_host_render_ms: Option<f64>,
) -> u32 {
    const MAX_FRAMES_PER_TICK: u32 = 5;

    let Some(run_ms) = average_run_frame_ms.filter(|time| time.is_finite() && *time > 0.0) else {
        return MAX_FRAMES_PER_TICK;
    };
    let render_ms = average_host_render_ms
        .filter(|time| time.is_finite() && *time >= 0.0)
        .unwrap_or(0.0);
    let available_script_ms = (frame_time_ms - render_ms).max(run_ms);
    ((available_script_ms / run_ms) as u32).clamp(1, MAX_FRAMES_PER_TICK)
}

#[cfg(test)]
mod render_aware_catch_up_tests {
    use super::calculate_max_frames_per_tick;

    #[test]
    fn render_overload_does_not_schedule_invisible_catch_up_frames() {
        assert_eq!(
            calculate_max_frames_per_tick(1000.0 / 60.0, Some(5.0), Some(80.0)),
            1,
            "an 80ms render has already exhausted the next 60Hz frame budget"
        );
    }

    #[test]
    fn light_rendering_keeps_the_existing_catch_up_capacity() {
        assert_eq!(
            calculate_max_frames_per_tick(1000.0 / 60.0, Some(2.0), Some(1.0)),
            5
        );
    }

    #[test]
    fn embedders_without_render_samples_keep_script_only_scheduling() {
        assert_eq!(
            calculate_max_frames_per_tick(1000.0 / 60.0, Some(5.0), None),
            3
        );
    }
}
    ...
    pub fn build(self) -> Arc<Mutex<Player>> {
        ...
        let player = Arc::new_cyclic(|self_ref| {
            Mutex::new(Player {
                ...
                recent_host_render_timings: VecDeque::with_capacity(10),
...
fn run_mouse_pick<'gc>(
    ...
    #[cfg(feature = "aether_metrics")]
    crate::aether_metrics::record_mouse_pick(started.elapsed());
