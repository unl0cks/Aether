...
fn compact_broadcast_entries<T>(
    entries: &mut Vec<T>,
    prune_stale: bool,
    mut classify: impl FnMut(&T) -> BroadcastEntryState,
) -> u64 {
    let mut stale_pruned = 0_u64;
    entries.retain(|entry| match classify(entry) {
        BroadcastEntryState::Active => true,
        BroadcastEntryState::Dead => false,
        BroadcastEntryState::Stale if prune_stale => {
            stale_pruned = stale_pruned.saturating_add(1);
            false
        }
        BroadcastEntryState::Stale => true,
    });
    stale_pruned
}
...
impl<'gc> Avm2<'gc> {
    ...
    pub fn broadcast_event(
        ...
        let prune_stale = crate::aether_performance::avm2_broadcast_fast_path_enabled();
        #[cfg(not(feature = "aether_performance"))]
        let prune_stale = false;
        #[cfg(feature = "aether_metrics")]
        let profile_enter_frame_handlers = profile_enter_frame_handlers
            && crate::aether_diagnostics::enter_frame_handler_attribution_enabled();
        #[cfg(not(feature = "aether_metrics"))]
        let profile_enter_frame_handlers = {
            let _ = profile_enter_frame_handlers;
            false
        };
        let mut stale_candidates: SmallVec<[Object<'gc>; 8]> = SmallVec::new();

        let execution_list =
            snapshot_broadcast_entries(context.avm2.broadcast_list.entry(event_name).or_default());

        for entry in execution_list {
            if let Some(object) = entry.upgrade(context.gc())
                && object.is_of_type(on_type.inner_class_definition())
            {
                #[cfg(feature = "aether_metrics")]
                crate::aether_metrics::avm2_broadcast_target_examined();

                let mut activation = Activation::from_nothing(context);

                if events::broadcast_event(
                    &mut activation,
                    object,
                    event,
                    ...
                    #[cfg(feature = "aether_metrics")]
                    crate::aether_metrics::avm2_broadcast_target_dispatched();
                } else if prune_stale {
                    stale_candidates.push(object);
                }
            }
        }

        let mut confirmed_stale: SmallVec<[Object<'gc>; 8]> = SmallVec::new();
        for object in stale_candidates {
            if !events::has_event_listener(context.gc(), object, event_name) {
                confirmed_stale.push(object);
            }
        }

        let gc_context = context.gc_context;
        let stale_pruned = compact_broadcast_entries(
            context.avm2.broadcast_list.entry(event_name).or_default(),
            prune_stale,
            |entry| match entry.upgrade(gc_context) {
                None => BroadcastEntryState::Dead,
                Some(object)
                    if confirmed_stale
                        .iter()
                        .any(|candidate| Object::ptr_eq(object, *candidate)) =>
                {
                    BroadcastEntryState::Stale
                }
                Some(_) => BroadcastEntryState::Active,
            },
        );

        #[cfg(feature = "aether_metrics")]
        for _ in 0..stale_pruned {
            crate::aether_metrics::avm2_broadcast_stale_registration_pruned();
        }
        #[cfg(not(feature = "aether_metrics"))]
        let _ = stale_pruned;
    }
...
mod tests {
    ...
    #[test]
    fn broadcast_iteration_snapshot_survives_bucket_compaction() {
        let mut entries = vec![1_u8, 2, 3];
        let snapshot = snapshot_broadcast_entries(&entries);
    ...
    #[test]
    fn broadcast_iteration_snapshot_excludes_late_registrations() {
        let mut entries = vec![1_u8, 2];
        let snapshot = snapshot_broadcast_entries(&entries);
    ...
    #[test]
    fn broadcast_compaction_always_removes_dead_entries() {
        let mut entries = vec![1_u8, 2, 3];
        let pruned = compact_broadcast_entries(&mut entries, false, |entry| match entry {
            2 => BroadcastEntryState::Dead,
            _ => BroadcastEntryState::Active,
        });
    ...
    #[test]
    fn broadcast_compaction_keeps_stale_entries_when_fast_path_is_disabled() {
        let mut entries = vec![1_u8, 2];
        let pruned = compact_broadcast_entries(&mut entries, false, |entry| match entry {
            2 => BroadcastEntryState::Stale,
            _ => BroadcastEntryState::Active,
        });
    ...
    #[test]
    fn broadcast_compaction_prunes_only_confirmed_stale_entries() {
        let mut entries = vec![1_u8, 2, 3];
        let pruned = compact_broadcast_entries(&mut entries, true, |entry| match entry {
            2 => BroadcastEntryState::Stale,
            _ => BroadcastEntryState::Active,
        });
        assert_eq!(entries, vec![1, 3]);
