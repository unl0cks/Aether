...
impl<'gc> Stage<'gc> {
    ...
    pub fn broadcast_render(self, context: &mut UpdateContext<'gc>) {
        ...
        Avm2::broadcast_event(context, render_evt, dobject_constr, false);
...
impl<'gc> TDisplayObject<'gc> for Stage<'gc> {
    ...
    fn enter_frame(self, context: &mut UpdateContext<'gc>) {
        #[cfg(feature = "aether_metrics")]
        let stage_enter_started = Instant::now();
        if self.begin_avm2_lifecycle_traversal(crate::display_object::Avm2LifecycleTraversal::Enter)
        {
            for child in self.iter_render_list() {
                child.enter_frame(context);
                if child
                    .is_avm2_lifecycle_dirty(crate::display_object::Avm2LifecycleTraversal::Enter)
                {
                    self.mark_avm2_lifecycle_dirty_local(
                        crate::display_object::Avm2LifecycleTraversal::Enter,
                    );
                }
            }
        }
        ...
        crate::aether_metrics::record_tick_phase(
            crate::aether_metrics::TickPhase::Avm2EnterStage,
            stage_enter_started.elapsed(),
        );
        ...
        #[cfg(feature = "aether_metrics")]
        crate::aether_metrics::record_tick_phase(
            crate::aether_metrics::TickPhase::Avm2EnterBroadcast,
    ...
    fn construct_frame(self, context: &mut UpdateContext<'gc>) {
        ...
        for child in self.iter_render_list() {
            ...
            if child
                .is_avm2_lifecycle_dirty(crate::display_object::Avm2LifecycleTraversal::Construct)
            {
                self.mark_avm2_lifecycle_dirty_local(
                    crate::display_object::Avm2LifecycleTraversal::Construct,
                );
