...
use crate::display_object::{
    Avm1Button, Avm1TextFieldBinding, Avm2Button, Avm2LifecycleTraversal, BoundsMode,
    DisplayObjectBase, DisplayObjectPtr, EditText, Graphic, MorphShape, Text, Video,
};
...
fn classify_avm2_explicit_goto(
    is_action_script_3: bool,
    current_frame: FrameNumber,
    target_frame: FrameNumber,
) -> Option<crate::aether_metrics::Avm2ExplicitGotoKind> {
    is_action_script_3.then_some(if current_frame == target_frame {
        crate::aether_metrics::Avm2ExplicitGotoKind::SameFrame
    } else {
        crate::aether_metrics::Avm2ExplicitGotoKind::ChangedFrame
    })
}
...
impl<'gc> MovieClip<'gc> {
    ...
    pub fn queued_script_frame(self) -> FrameNumber {
    ...
    fn no_op_goto(self, context: &mut UpdateContext<'gc>) {
        if self.movie().is_action_script_3() {
            #[cfg(feature = "aether_metrics")]
            let profile_nested_goto =
                crate::aether_metrics::begin_avm2_explicit_goto_profile(|| {
                    classify_avm2_explicit_goto(true, self.current_frame(), self.current_frame())
                });
            #[cfg(not(feature = "aether_metrics"))]
            let profile_nested_goto = false;
            ...
            run_inner_goto_frame(context, &[], self, profile_nested_goto);
        }
    }
    ...
    fn run_frame_internal(
        ...
        let current_frame = self.0.current_frame.get();
        self.0.queued_script_frame.set(current_frame);
        if self.last_queued_script_frame() != Some(current_frame) {
            ...
            self.set_has_pending_script(self.has_frame_script(current_frame));
        }
    }
    ...
    fn run_goto(mut self, context: &mut UpdateContext<'gc>, frame: FrameNumber, is_implicit: bool) {
        ...
        self.set_skip_next_enter_frame(false);
        ...
        if !is_implicit {
            ...
            run_inner_goto_frame(context, &removed_frame_scripts, self, profile_nested_goto);
        }
            ...
            && self.has_pending_script()
        {
                ...
                .contains_flag(MovieClipFlags::EXECUTING_AVM2_FRAME_SCRIPT)
            {
                ...
                if is_fresh_frame && let Some(callable) = self.frame_script(frame_id) {
                    ...
                    {
                        let summary = crate::aether_compatibility::rebind_named_timeline_children(
                            context, self,
                        );
                        #[cfg(feature = "aether_diagnostics")]
                        crate::aether_diagnostics::record_timeline_child_rebind(
                    ...
                    // The current frame either has no callable anymore or its
                    // script already ran. The old whole-tree construct pass
                    // refreshed this flag every time; with dirty traversal it
                    // must be consumed here so an inert clip can become clean.
                    self.set_has_pending_script(false);
                }
            }
        }
...
impl<'gc> TDisplayObject<'gc> for MovieClip<'gc> {
    ...
    fn enter_frame(self, context: &mut UpdateContext<'gc>) {
        if !self.begin_avm2_lifecycle_traversal(Avm2LifecycleTraversal::Enter) {
            return;
        }
        ...
        for child in self.iter_render_list().rev() {
            if skip_frame {
                ...
                child.set_skip_next_enter_frame(true);
            }
            child.enter_frame(context);
            if child.is_avm2_lifecycle_dirty(Avm2LifecycleTraversal::Enter) {
                self.mark_avm2_lifecycle_dirty_local(Avm2LifecycleTraversal::Enter);
            }
        }
        ...
        if self.movie().is_action_script_3() {
            ...
            if self.playing() || !self.0.queued_tags.borrow().is_empty() {
                self.mark_avm2_lifecycle_dirty_local(Avm2LifecycleTraversal::Enter);
            }
        }
    }
    ...
    fn construct_frame(self, context: &mut UpdateContext<'gc>) {
        if !self.begin_avm2_lifecycle_traversal(Avm2LifecycleTraversal::Construct) {
            return;
        }

        // AVM1 code expects to execute in line with timeline instructions, so
        // it's exempted from frame construction.
        let is_avm2 = self.movie().is_action_script_3();
        let frame_is_available = self.frames_loaded() >= 1 || self.header_frames() == 0;
        if is_avm2 && frame_is_available {
                ...
                for child in self.iter_render_list() {
                    ...
                    if child.object2().is_none() {
                        ...
                        if running_construct_frame {
                            self.mark_avm2_lifecycle_dirty_local(Avm2LifecycleTraversal::Construct);
                            continue;
                        }
                    ...
                    if child.is_avm2_lifecycle_dirty(Avm2LifecycleTraversal::Construct) {
                        self.mark_avm2_lifecycle_dirty_local(Avm2LifecycleTraversal::Construct);
                    }
                }
            }
        } else if is_avm2 {
            // Streaming can make frame 1 constructible later. Keep this
            // subtree scheduled until the required data is available.
            self.mark_avm2_lifecycle_dirty_local(Avm2LifecycleTraversal::Construct);
        }
    ...
    fn set_object2(self, context: &mut UpdateContext<'gc>, to: Avm2StageObject<'gc>) {
        ...
        {
            let class_name = to.instance_class().name();
            let exact_avatar_local_name = if class_name.local_name().as_wstr() == b"AvatarMC" {
                b"AvatarMC".as_slice()
            } else {
                b"".as_slice()
            };
            self.base().set_aether_adaptive_avatar_cache_root_candidate(
                crate::aether_performance::is_aqw_avatar_cache_candidate(
                    self.movie().url(),
                    exact_avatar_local_name,
                    class_name.namespace().is_public(),
                ),
            );
            DisplayObject::from(self).refresh_aether_adaptive_avatar_cache_candidates();
        }
...
mod aether_nested_goto_tests {
    use super::classify_avm2_explicit_goto;
    use crate::aether_metrics::Avm2ExplicitGotoKind;

    #[test]
    fn explicit_goto_classification_distinguishes_avm2_same_and_changed_frames() {
        assert_eq!(
            classify_avm2_explicit_goto(true, 7, 7),
            Some(Avm2ExplicitGotoKind::SameFrame)
        );
        assert_eq!(
            classify_avm2_explicit_goto(true, 7, 8),
            Some(Avm2ExplicitGotoKind::ChangedFrame)
        );
    }

    #[test]
    fn explicit_goto_classification_excludes_avm1() {
        assert_eq!(classify_avm2_explicit_goto(false, 7, 7), None);
        assert_eq!(classify_avm2_explicit_goto(false, 7, 8), None);
    }
}
