    ...
    + Into<DisplayObject<'gc>>
{
    ...
    fn render_children(self, context: &mut RenderContext<'_, 'gc>) {
        ...
        for child in self.iter_render_list() {
            ...
            if !child.visible() {
                #[cfg(feature = "aether_metrics")]
                crate::aether_metrics::container_child_invisible();
            }
            ...
            if child.clip_depth() > 0 && child.allow_as_mask() {
                #[cfg(feature = "aether_metrics")]
                crate::aether_metrics::container_mask_rendered();
...
#[cfg(test)]
mod aether_filterless_direct_tests {
    use super::*;
    use crate::display_object::{
        TDisplayObject, filterless_direct_render_subtree_is_semantically_safe,
    };
    use gc_arena::arena::rootless_mutate;
    use std::sync::Arc;

    fn append_test_child<'gc>(parent: MovieClip<'gc>, child: MovieClip<'gc>, mc: &Mutation<'gc>) {
        parent.raw_container_mut(mc).push_id(child.into());
    }

    #[test]
    fn direct_filterless_cache_bypass_accepts_plain_normal_subtree() {
        rootless_mutate(|mc| {
            let movie = Arc::new(SwfMovie::empty(10, None));
            let parent = MovieClip::new(movie.clone(), mc);
            let child = MovieClip::new(movie, mc);
            append_test_child(parent, child, mc);

            assert!(filterless_direct_render_subtree_is_semantically_safe(
                parent.into()
            ));
        });
    }
    ...
    #[test]
    fn direct_filterless_cache_bypass_rejects_descendant_blend_grouping() {
        rootless_mutate(|mc| {
            let movie = Arc::new(SwfMovie::empty(10, None));
            let parent = MovieClip::new(movie.clone(), mc);
            let child = MovieClip::new(movie, mc);
            child.set_blend_mode(swf::BlendMode::Multiply.into());
            append_test_child(parent, child, mc);
    ...
    #[test]
    fn direct_filterless_cache_bypass_rejects_descendant_timeline_mask() {
        rootless_mutate(|mc| {
            let movie = Arc::new(SwfMovie::empty(10, None));
            let parent = MovieClip::new(movie.clone(), mc);
            let child = MovieClip::new(movie, mc);
            child.set_clip_depth(10);
            append_test_child(parent, child, mc);
