...
impl<'gc> TDisplayObject<'gc> for Avm2Button<'gc> {
    ...
    fn enter_frame(self, context: &mut UpdateContext<'gc>) {
        if !self
            .begin_avm2_lifecycle_traversal(crate::display_object::Avm2LifecycleTraversal::Enter)
        {
            return;
        }
        ...
        for state in self.all_state_children(false) {
            ...
            if state.is_avm2_lifecycle_dirty(crate::display_object::Avm2LifecycleTraversal::Enter) {
    ...
    fn run_frame_scripts(self, context: &mut UpdateContext<'gc>) {
        if !self.begin_avm2_lifecycle_traversal(
            crate::display_object::Avm2LifecycleTraversal::FrameScripts,
        ) {
            return;
        }
        ...
        for state in self.all_state_children(self.0.weird_framescript_order.take()) {
            ...
            if state.is_avm2_lifecycle_dirty(
                crate::display_object::Avm2LifecycleTraversal::FrameScripts,
            ) {
                self.mark_avm2_lifecycle_dirty_local(
                    crate::display_object::Avm2LifecycleTraversal::FrameScripts,
