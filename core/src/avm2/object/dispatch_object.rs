...
impl<'gc> DispatchObject<'gc> {
    ...
    /// The dispatch list, for reading.
    ///
    /// Separate from [`Self::dispatch_mut`] because asking whether anything is listening happens
    /// far more often than registering a listener, and must not require a mutation.
    pub fn dispatch(self) -> Ref<'gc, DispatchList<'gc>> {
        Gc::as_ref(self.0).dispatch.borrow()
    }

    pub fn dispatch_mut(self, mc: &Mutation<'gc>) -> RefMut<'gc, DispatchList<'gc>> {
        unlock!(Gc::write(mc, self.0), DispatchObjectData, dispatch).borrow_mut()
    }
}
