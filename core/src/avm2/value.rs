...
#[inline]
fn call_method_dispatch<T>(
    needs_arguments_object: bool,
    get_bound_method: impl FnOnce() -> Option<T>,
) -> CallMethodDispatch<T> {
    if !needs_arguments_object {
        CallMethodDispatch::Direct
    } else if let Some(bound_method) = get_bound_method() {
        CallMethodDispatch::Cached(bound_method)
    } else {
        CallMethodDispatch::BindAndCache
    }
}
...
impl<'gc> Value<'gc> {
    ...
    pub fn call_method_with_args(
        ...
        match call_method_dispatch(full_method.method.needs_arguments_object(), || {
            self.as_object()
                .and_then(|object| object.get_bound_method(id))
        }) {
            // Execute immediately if this method doesn't require a bound callee.
            CallMethodDispatch::Direct => exec(
                full_method.method,
                full_method.scope(),
                *self,
...
mod tests {
    ...
    #[test]
    fn callmethod_without_arguments_object_skips_bound_cache() {
        let probes = Cell::new(0);

        let dispatch = call_method_dispatch(false, || {
            probes.set(probes.get() + 1);
            Some(42)
        });

        assert_eq!(dispatch, CallMethodDispatch::Direct);
        assert_eq!(probes.get(), 0);
    }

    #[test]
    fn callmethod_with_arguments_object_preserves_callee_and_binding_fallback() {
        let probes = Cell::new(0);

        let cached = call_method_dispatch(true, || {
            probes.set(probes.get() + 1);
            Some(42)
        });
        let missing = call_method_dispatch(true, || {
            probes.set(probes.get() + 1);
            None::<i32>
        });

        assert_eq!(cached, CallMethodDispatch::Cached(42));
        assert_eq!(missing, CallMethodDispatch::BindAndCache);
