...
const INLINE_NATIVE_ARGUMENTS: usize = 4;

type NativeArgumentBuffer<'gc> = SmallVec<[Value<'gc>; INLINE_NATIVE_ARGUMENTS]>;

fn native_argument_buffer<'gc>(expected_len: usize) -> NativeArgumentBuffer<'gc> {
    SmallVec::with_capacity(expected_len)
}

fn append_argument_tail<'gc>(
    arguments_list: &mut NativeArgumentBuffer<'gc>,
    user_arguments: FunctionArgs<'_, 'gc>,
    start: usize,
) {
    for index in start..user_arguments.len() {
        arguments_list.push(user_arguments.get_at(index));
    }
}
...
impl<'a, 'gc> Activation<'a, 'gc> {
    ...
    pub fn resolve_parameters(
    ...
    ) -> Result<NativeArgumentBuffer<'gc>, Error<'gc>> {
        let mut arguments_list = native_argument_buffer(user_arguments.len().max(signature.len()));
        ...
        match user_arguments.len().cmp(&signature.len()) {
            Ordering::Greater => {
                ...
                append_argument_tail(&mut arguments_list, user_arguments, signature.len())
            }
            Ordering::Less => {
    ...
    pub fn init_from_method(
        ...
        // `Stack::get_stack_frame` already initializes all local registers to
        // undefined, so we just have to advance past them. Verified bytecode
        // writes operand-stack slots before reading them.
        self.reset_stack();
...
#[cfg(test)]
mod tests {
    use super::{append_argument_tail, native_argument_buffer};
    use crate::avm2::function::FunctionArgs;
    use crate::avm2::value::Value;
    use std::cell::Cell;

    #[test]
    fn native_argument_buffer_keeps_four_values_inline() {
        let values = [Value::Undefined; 4];
        let mut arguments = native_argument_buffer(values.len());
        arguments.extend_from_slice(&values);

        assert_eq!(arguments.len(), 4);
        assert!(!arguments.spilled());
        assert_eq!(arguments.as_slice(), values.as_slice());
    }

    #[test]
    fn native_argument_buffer_spills_for_five_values() {
        let values = [Value::Undefined; 5];
        let mut arguments = native_argument_buffer(values.len());
        arguments.extend_from_slice(&values);
    ...
    #[test]
    fn native_argument_tail_preserves_value_argument_order() {
        let values = [Value::Integer(10), Value::Integer(20), Value::Integer(30)];
        let mut arguments = native_argument_buffer(values.len() - 1);

        append_argument_tail(&mut arguments, FunctionArgs::from_slice(&values), 1);

        assert_eq!(arguments.as_slice(), &values[1..]);
    }

    #[test]
    fn native_argument_tail_preserves_cell_argument_order() {
        let values = [Value::Integer(10), Value::Integer(20), Value::Integer(30)];
        let cells = values.map(Cell::new);
        let mut arguments = native_argument_buffer(values.len() - 1);

        append_argument_tail(&mut arguments, FunctionArgs::from_cell_slice(&cells), 1);
