...
fn initialize_locals<'gc>(data: &[Cell<Value<'gc>>], num_locals: usize) {
    for value in &data[..num_locals] {
...
impl<'gc> Stack<'gc> {
    ...
    pub fn get_stack_frame(&self, method: Method<'gc>) -> StackFrame<'_, 'gc> {
        ...
        // Locals can be read before assignment, so clear stale values from
        // previous frames. Verified AVM2 bytecode writes operand-stack slots
        // before reading them, and the stack is not traced while frames exist.
        initialize_locals(subslice, body.num_locals as usize);
...
mod tests {
    ...
    #[test]
    fn frame_initialization_clears_locals_without_exposing_dirty_operand_capacity() {
        const NUM_LOCALS: usize = 2;
        let data = [
            Cell::new(Value::Integer(10)),
            Cell::new(Value::Integer(11)),
            Cell::new(Value::Integer(90)),
            Cell::new(Value::Integer(91)),
            Cell::new(Value::Integer(92)),
        ];

        initialize_locals(&data, NUM_LOCALS);

        assert!(matches!(data[0].get(), Value::Undefined));
        assert!(matches!(data[1].get(), Value::Undefined));
        assert!(matches!(data[2].get(), Value::Integer(90)));
        assert!(matches!(data[3].get(), Value::Integer(91)));
        assert!(matches!(data[4].get(), Value::Integer(92)));

        let frame = StackFrame::for_data(&data);
        frame.set_stack_pointer(NUM_LOCALS);
        frame.push(Value::Integer(20));
        frame.push(Value::Integer(21));
        frame.push(Value::Integer(22));

        assert!(matches!(frame.pop(), Value::Integer(22)));
        assert!(matches!(frame.pop(), Value::Integer(21)));
        assert!(matches!(frame.pop(), Value::Integer(20)));
    }
}
